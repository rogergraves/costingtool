...
describe ClicksLookup do
  before do
    @lookup_table = ClicksLookupTable.create(:description => 'Indigo 10000')
    @lookup_table_row = ClicksLookup.create({
      :clicks_lookup_table_id => @lookup_table.id,
      :label => "Tier 1",
      :color_range_start => 3,
      :color_range_end => 6,
      :black => 1,
      :volume_range_start => 1,
      :volume_range_end => 4,
      :price => 10.0,
    })
  end
